/**
 * PDF Metadata Editor
 *
 * Модуль PDF Metadata Editor для EvolutionCMS. Редактирования метаданных PDF файла.
 *
 * @category     module
 * @version      1.0.3
 * @internal     @modx_category Manager and Admin
 * @internal     @properties
 * @internal     @icon fa fa-file-pdf
 * @homepage     https://github.com/ProjectSoft-STUDIONIONS/module-pdf-metadata#readme
 * @license      https://github.com/ProjectSoft-STUDIONIONS/module-pdf-metadata/blob/main/LICENSE GPL-3.0 License (GPL-3.0)
 * @reportissues https://github.com/ProjectSoft-STUDIONIONS/module-pdf-metadata/issues
 * @author       Чернышёв Андрей aka ProjectSoft <projectsoft2009@yandex.ru>
 * @lastupdate   2026-04-18
 */

/**
 * PDF Metadata Editor
 *
 * Модуль PDF Metadata Editor для EvolutionCMS. Редактирования метаданных PDF файла.
 *
 * @category     module
 * @version      1.0.3
 * @internal     @modx_category Manager and Admin
 * @internal     @properties
 * @internal     @icon fa fa-file-pdf
 * @homepage     https://github.com/ProjectSoft-STUDIONIONS/module-pdf-metadata#readme
 * @license      https://github.com/ProjectSoft-STUDIONIONS/module-pdf-metadata/blob/main/LICENSE GPL-3.0 License (GPL-3.0)
 * @reportissues https://github.com/ProjectSoft-STUDIONIONS/module-pdf-metadata/issues
 * @author       Чернышёв Андрей aka ProjectSoft <projectsoft2009@yandex.ru>
 * @lastupdate   2026-04-18
 */

require_once MODX_BASE_PATH . 'assets/modules/pdf-metadata/module.pdf-metadata.php';
